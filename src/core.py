...
def create_dummy_variables(df: pd.DataFrame, date_col: str, policy_date: str) -> pd.DataFrame:
...
def plot_policy_effect(df: pd.DataFrame, y_col: str, date_col: str, policy_date: str, title: str, output_path: Path, plot: bool=False):
    ...
    ax.plot(pre_policy[date_col], pre_policy[y_col], label='Pre-Policy', color='#4A90A4', linewidth=1.2)
    ax.plot(post_policy[date_col], post_policy[y_col], label='Post-Policy', color='#D4A574', linewidth=1.2)
