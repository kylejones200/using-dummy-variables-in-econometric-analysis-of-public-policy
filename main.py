...
def main():
    parser = argparse.ArgumentParser(
        description="Dummy Variables in Econometric Policy Analysis"
    )
    ...
    if args.data_path and args.data_path.exists():
        ...
        policy_idx = (
            dates.get_loc(policy_date) if policy_date in dates else len(dates) // 2
        )
        ...
        df = create_dummy_variables(
            df, config["data"]["date_column"], config["policy"]["policy_date"]
        )
        x_cols = ["policy_dummy"]
